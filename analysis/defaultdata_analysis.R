...
ggplot(data = Default) +
  aes(balance, binary_def) +
  geom_point(alpha = 1/5, aes(color = factor(binary_def))) +
  geom_smooth(method = "glm", 
              method.args = list(family = "binomial"), 
              se = FALSE, color = 'black') +
  xlab("Balance") + 
  ylab("Default on Loan") + 
  ggtitle("Defaulting on a Loan and Balance")+
  labs("Default") +
  scale_color_manual(labels = c("No", "Yes"), values = c('lightblue', 'red'))
...
probit_fscore
probit_error <- (probit_confusion[1, 2] + logistic_confusion[2, 1])/nrow(testing)
probit_error
